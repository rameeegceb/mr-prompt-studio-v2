...
import FrameworkRenderer from "./renderers/FrameworkRenderer";
import { buildFrameworkExplorer } from "../utils/frameworkExplorer";

export default function FrameworkExplorerDashboard({ course }) {
	const frameworks = useMemo(
		() => buildFrameworkExplorer(course),
		[course]
	);
	...
	if (frameworks.length === 0) {
		return (
			<section className="rounded-2xl border border-dashed border-slate-300 bg-white p-8 text-center text-slate-500 shadow-sm">
				No frameworks were found in the current course content.
			</section>
		);
	}

	const totalExamples = frameworks.reduce(
		(count, framework) => count + framework.examples.length,
		0
	);
		...
		<section className="space-y-6">
			<div className="grid gap-4 md:grid-cols-3">
				...
				<div className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
					...
					<p className="mt-1 text-sm text-slate-500">
						Explore reusable framework patterns without opening a lesson.
					</p>
				</div>
			</div>
			...
			<div className="grid gap-6 xl:grid-cols-[minmax(0,1.15fr)_minmax(360px,0.85fr)]">
				...
				<div className="xl:sticky xl:top-6 xl:self-start">
					<FrameworkRenderer framework={selectedFramework} />
				</div>
			</div>
		</section>
	);
